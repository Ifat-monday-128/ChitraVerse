...
export default function AdminManagement({ section, userId, changed }: { section: Section; userId: number; changed: () => void }) {
  ...
  return <section className="management" aria-label={`${section} management`}>
    ...
    {editing && <Dialog busy={busy} title={editing.title_id ? 'Edit title' : 'Add title'} close={() => { if (!busy) { setEditing(null); setError(''); } }}><form className="management-editor" onSubmit={save}><p className="eyebrow">CATALOG EDITOR</p><h2>{editing.title_id ? 'Edit title' : 'Add a new title'}</h2><p>Use verified metadata. Community ratings and imported TMDB scores cannot be edited here.</p>{error && <p role="alert" className="admin-feedback error">{error}</p>}<fieldset disabled={busy}>
