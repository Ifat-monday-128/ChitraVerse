...
export default function PopularInterests({ openGenre }: { openGenre: (id: number) => void }) {
  ...
  const track = useMarqueeSpeed<HTMLUListElement>();
  useEffect(() => {
  ...
  return <section className="discovery-section interests-section" aria-labelledby="interests-heading">
    <div className="discovery-heading"><div><p className="eyebrow">FIND YOUR KIND OF STORY</p><h2 id="interests-heading">Popular <span>interests</span></h2><p>Explore your library by genre · Largest collections first</p></div></div>
    {error ? <div className="discovery-message" role="alert">Genres couldn’t be loaded. <button onClick={() => { setError(false); setRetry(value => value + 1); }}>Try again</button></div>
      ...
      : <div className="home-marquee interests-marquee"><ul className="home-marquee-track interests-marquee-track" id="interests-rail" ref={track}>{[...items, ...items].map((item, index) => <li key={`${item.genre_id}-${index}`}><button className="interest-card" onClick={() => openGenre(item.genre_id)} aria-label={`Explore ${item.name}, ${item.title_count} ${item.title_count === 1 ? 'title' : 'titles'}`}>
